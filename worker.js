    ...
    if (
      ...
      if (request.method === 'DELETE') {
        try {
          ...
          const filtered = orders.filter(order => String(order.id) !== String(body.id));
          if (filtered.length === orders.length) return json({ error: 'الطلب غير موجود' }, 404);
          await env.GREEN_MOON_KV.put(ORDERS_KEY, JSON.stringify(filtered));
