...
const SEED_PRODUCTS = [
  ...
  {id:2,name:'أجلونيما بينك سبوت',details:'أوراق وردية كثيفة بتوزيعات خضراء جميلة.',price:0,image:'/assets/product-2.jpg'},
  {id:3,name:'أجلونيما بينك جرين',details:'أجلونيما بأوراق وردية زاهية وتفاصيل خضراء.',price:0,image:'/assets/product-3.jpg'},
  {id:4,name:'تشكيلة أجلونيما',details:'مجموعة من نباتات أجلونيما بألوان ونقوش مختلفة.',price:0,image:'/assets/product-4.jpg'},
  {id:5,name:'أجلونيما بينك فين',details:'أوراق خضراء بنقوش وردية وعروق وردية واضحة.',price:0,image:'/assets/product-5.jpg'},
  {id:6,name:'أجلونيما وايت',details:'أجلونيما بأوراق خضراء وبيضاء، مناسبة للمكاتب والبيوت.',price:0,image:'/assets/product-6.jpg'},
  {id:7,name:'بوتس مبرقش',details:'بوتس أخضر بتبرقش فاتح، نبات سهل العناية وسريع النمو.',price:0,image:'/assets/product-7.jpg'},
  {id:8,name:'أنثوريوم أبيض',details:'أنثوريوم بأزهار بيضاء وأوراق خضراء أنيقة.',price:0,image:'/assets/product-8.jpg'}
];
...
export default {
 async fetch(request,env){
  ...
  if(url.pathname==='/api/products' && request.method==='GET') return json(await getProducts(env));
  if(url.pathname==='/api/logo' && request.method==='GET') return json({logo:await getLogo(env)});
  if(url.pathname==='/api/settings' && request.method==='GET') return json(await env.GREEN_MOON_KV.get(SETTINGS_KEY,'json')||{storeName:'Green Moon Plants and Flowers',whatsapp:''});

  // العميل ينشئ طلبًا، ويُحفظ الطلب على Cloudflare KV قبل فتح واتساب.
...
const total=productsTotal+shipping; 
   ...
   }catch(e){return json({error:'تعذر حفظ الطلب'},500)}
  }

  if(url.pathname.startsWith('/api/admin/')){
   if(!adminOk(request,env)) return json({error:'غير مصرح. كلمة مرور لوحة التحكم غير صحيحة.'},401);
   if(url.pathname==='/api/admin/products' && request.method==='PUT'){
    const products=await request.json(); if(!Array.isArray(products)) return json({error:'بيانات المنتجات غير صحيحة'},400);
    await env.GREEN_MOON_KV.put(PRODUCTS_KEY,JSON.stringify(products)); return json({ok:true,products});
   }
   if(url.pathname==='/api/admin/logo' && request.method==='PUT'){
    const body=await request.json(); if(!body.logo) return json({error:'اللوجو مفقود'},400);
    await env.GREEN_MOON_KV.put(LOGO_KEY,body.logo); return json({ok:true,logo:body.logo});
   }
   if(url.pathname==='/api/admin/settings' && request.method==='PUT'){
    const body=await request.json(); await env.GREEN_MOON_KV.put(SETTINGS_KEY,JSON.stringify(body)); return json({ok:true,settings:body});
   }
   if(url.pathname==='/api/admin/orders' && request.method==='GET') return json(await getOrders(env));
   if(url.pathname==='/api/admin/orders' && request.method==='PUT'){
    const body=await request.json(); const orders=await getOrders(env); const i=orders.findIndex(o=>o.id===body.id); if(i<0)return json({error:'الطلب غير موجود'},404); if(body.status)orders[i].status=String(body.status).slice(0,40); await env.GREEN_MOON_KV.put(ORDERS_KEY,JSON.stringify(orders)); return json({ok:true,order:orders[i]});
   }
   if(url.pathname==='/api/admin/orders' && request.method==='DELETE'){
    const id=url.searchParams.get('id'); const orders=await getOrders(env); const next=orders.filter(o=>o.id!==id); await env.GREEN_MOON_KV.put(ORDERS_KEY,JSON.stringify(next)); return json({ok:true});
   }
   if(url.pathname==='/api/admin/reset' && request.method==='POST'){
    await env.GREEN_MOON_KV.put(PRODUCTS_KEY,JSON.stringify(SEED_PRODUCTS)); await env.GREEN_MOON_KV.put(LOGO_KEY,'/assets/logo.jpg'); return json({ok:true});
   }
  }
